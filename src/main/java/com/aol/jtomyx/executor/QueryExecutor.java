...
public class QueryExecutor {
    ...
    public void executeAndWrite(String query, OutputStream os, String xsl, int getnumfound,
        ...
        timer.end();
        logger.info("Time taken to acquire a connection from datasource: " + timer.getTimeInMillis() + " ms");
        timer.reset();
        timer.start();
        Statement stmt = null;
        ...
        try {
            ...
            if (rs != null) {
                ...
                logger.info("Time taken to create statement and execute a query: " + qtime + " ms");
                timer.reset();
                timer.start();
                if (getnumfound == 1) {
                    stmt_numfound = null;
                ...
                timer.end();
                logger.info("Time taken to write XML results: " + timer.getTimeInMillis() + " ms");
                rs.close();
